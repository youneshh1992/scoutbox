...
| 12 | case at `under_review` with the strip's `REVIEW_PLAYER` act | a colleague moved the case to contact_planned | the strip performs `startReview` with the stale rev | 409 `ROOM_VERSION_CONFLICT`; the strip says the case changed and re-reads; the control that appears is the current act | one move | live B (P8), hardening live L1–L3 |
| 13 | a Room tab open | the user's role was removed | any act | 403 `ROOM_PERMISSION_REQUIRED` / `LIFECYCLE_NOT_PERMITTED`; the strip re-reads and drops the control | nothing | live C (P8), hardening live L4 |
| 14 | the agent's client page | the client ended the representation | any read or deep link | 403 with the rule; the line is gone on the next read | nothing | journey E2E F7; hardening K, live L5 |
