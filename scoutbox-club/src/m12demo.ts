...
const ASSESSMENTS: Assessment[] = [
  {
    ...
    context: { fixture: 'Eastport U23 v Harbour U23', date: '2026-08-30', minutesWatched: 90, viewing: 'live' },
    ratings: [
...
const OPPS: Opportunity[] = [{
  id: 'opp-d1', type: 'trial', title: 'U23 open trial — attackers', team: 'U23', category: 'mens', ageGroup: 'U23', role: 'Forward',
  description: 'Two-session look at pressing forwards for the development squad.', schedule: 'Sat 09:30, Eastport Training Ground',
...
const OBJECTIVES: Objective[] = [{
  ...
  sharing: { orgIds: ['org-eastport'] }, status: 'active',
}];
...
function dayFor(trialId: string, playerName = 'Elias Svensson', playerId = 'pl-svensson'): TrialDay {
  if (!TRIAL_DAYS.has(trialId)) {
    TRIAL_DAYS.set(trialId, {
      id: trialId, playerId, playerName, proposedDate: new Date(NOW + 6 * DAY).toISOString().slice(0, 10), venue: 'Eastport Training Ground', reportDueAt: NOW + 13 * DAY,
      staff: [
...
export const demoM12: M12Api = {
  ...
  playerObjectives: async (_s, playerId) => delay(OBJECTIVES.filter((o) => o.playerId === playerId && o.sharing.orgIds.includes('org-eastport'))),
