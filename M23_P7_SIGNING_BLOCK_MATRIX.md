...
| subject removed their account (`subjectRemovedAt` / player gone) | 409 `SIGNING_SUBJECT_REMOVED` | 409 | — (no session) | 409 `SIGNING_SUBJECT_REMOVED` | allowed | allowed | — | ✓; `onPlayerDeleted` nulls the player's names on parties and history and stamps the package `subjectRemovedAt` — it does not cancel (no new write about a person who left; the club may cancel) [corrected in P7.1, D-P71-8] | — |
...
- Expiry is derived at read time and never written; a party whose
  ...
  same way (R4, #29).
- Deleting a player (`onPlayerDeleted`) nulls their name on parties and
  history and stamps the package; every further club act is refused
  `SIGNING_SUBJECT_REMOVED`; no completed package and no `db.signings` row
  is touched. (P7.1 correction: the P7 text said the packages were cancelled;
  they are not — cancelling remains the club's own act.)
