...
| Site | What it is | Class | After P6 |
| --- | --- | --- | --- |
| `storeContract.mjs:132` `recruitmentOffers: { guarantee: 'optional' }` | the store name reserved since P2; "the journey reports offer.available=false" | **canonical future store** | becomes `{ guarantee: 'module', owner: 'm28' }`: created empty by `registerOffers` at registration on every boot (the M12–M16 pattern). **No migration** — the schema stays at 2307, nothing is backfilled, and no lifecycle state is reinterpreted as an Offer (§4, §60–§62; decision recorded in M23_P6_OFFER_MODEL.md §2) |
| `m23/journey.mjs:16, 47, 260–263, 277, 320` | the journey projects `offer: { available, records:[{id,type,status}] }` and derives `offerAwaitingResponse` from `status === 'sent'` | read-only, migration compatibility | the projection reads the canonical rows through the domain's own derivations (`offerStatus`, `liveStatus`); `offerAwaitingResponse` is true only while the LIVE revision (the latest issued one) is ISSUED and unexpired at the request's clock |
