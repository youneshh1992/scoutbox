...
const M = {
  ...
  journey_evidence_funnel: meta('journey_evidence_funnel', 'pipeline', 'Journey stages (from the records)', 'rooms', 'window_entry', ['recruitmentCases', 'recruitmentContacts', 'requests', 'trials', 'assessments', 'roomDecisions', 'recruitmentOffers', 'signingPackages', 'signings'],
    'Each stage counts cases for which the record itself exists — a delivered contact, a completed trial, a submitted assessment, a finalized decision, an issued or answered Offer, a completed signing — not cases whose status merely says so. A case counts once per stage however many retries or revisions it carried. Cases whose history reached a stage with no record behind it are shown apart as "history only"; they are older cases, not progress. A recent window is incomplete by construction.'),
...
function buildDashboard(filters: DashboardFilters): Dashboard {
  ...
  const data: Record<string, Family> = {
    pipeline: fam('pipeline', {
      ...
      journey_evidence_funnel: {
        ...M.journey_evidence_funnel,
        ...count(8),
        cohort: count(8),
        rows: [{ stage: 'contacted', value: 6, historyOnly: 0, basis: 'case', source: 'recruitmentContacts' }, { stage: 'trial_requested', value: 4, historyOnly: 0, basis: 'case', source: 'requests' }, { stage: 'trial_scheduled', value: 3, historyOnly: 0, basis: 'case', source: 'trials' }, { stage: 'trial_completed', value: 3, historyOnly: 0, basis: 'case', source: 'trials' }, { stage: 'assessed', value: 3, historyOnly: 0, basis: 'case', source: 'assessments' }, { stage: 'decision_progress', value: 2, historyOnly: 0, basis: 'case', source: 'roomDecisions' }, { stage: 'decision_hold', value: 0, historyOnly: 0, basis: 'case', source: 'roomDecisions' }, { stage: 'decision_reject', value: 1, historyOnly: 0, basis: 'case', source: 'roomDecisions' }, { stage: 'offer_issued', value: 2, historyOnly: 0, basis: 'case', source: 'recruitmentOffers' }, { stage: 'offer_accepted', value: 1, historyOnly: 0, basis: 'case', source: 'recruitmentOffers' }, { stage: 'offer_declined', value: 0, historyOnly: 0, basis: 'case', source: 'recruitmentOffers' }, { stage: 'signing_started', value: 1, historyOnly: 0, basis: 'case', source: 'signingPackages' }, { stage: 'signed', value: 1, historyOnly: 0, basis: 'case', source: 'signings' }],
        intervals: { contact_to_trial: { n: 3, medianDays: 6 }, trial_completed_to_decision: { n: 3, medianDays: 4.5 }, decision_to_offer: { n: 2, medianDays: 3 }, offer_issued_to_accepted: { n: 1, medianDays: 2 }, accepted_to_signed: { n: 1, medianDays: 9 } },
        counting: 'case',
