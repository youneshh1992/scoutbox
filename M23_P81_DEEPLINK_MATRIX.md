...
| --- | --- | --- | --- |
| Room `#/recruitment/rooms/:id/:tab` | the opener's role removed from the room (restricted room) | 403 `ROOM_RESTRICTED` / `ROOM_PERMISSION_REQUIRED` on the room read | the Room says it is not open to them; the list re-reads (live C; hardening live L5) |
...
| Agent client `#/clients/:rel/:tab` | the representation ended / expired / disputed; the licence lapsed; the affiliation removed | 403 with the rule (`decision.code`) on the journey, Offers and signings routes; the agreement row reads by status | the client page says the record is not open to them; the journey line is gone (live G7; hardening K, hardening live L7) |
...
| Browser back / forward into any of the above | — | every screen re-reads on `pageshow` / focus / visibility (P8.1 app-level refetch; the strip's own re-read) | the stale control is gone before it can be used (live D8–D9; hardening live L3–L4) |
