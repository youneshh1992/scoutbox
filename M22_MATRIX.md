...
| F2 | **Global image scale fabricating a touch — found twice, by two holdout generations.** With the ball and player in a completely fixed relationship, an 18% camera zoom pulse produced a touch (generation 1). The first fix — normalising each sample by its own ball diameter before differencing — cancelled global scale exactly but missed frame-edge clipping of the player region, so step zooms produced three more (generation 2); it had also amplified ball-diameter noise by \|rel\|/d ≈ 3.5, manufacturing two extra touches at the smallest frame size and breaking resolution invariance. | `m22/track.mjs` (tracking layer) | One structural test, `EVENT_RULES.maxFrameScaleStepPerSample` = 4%, **derived** from the impulse threshold, the working range of \|rel\|/d and the maximum supported cadence — replacing a 1.4× guard that was asking an identity question where the question that mattered was thirty times smaller. Catches camera zoom, player approach and edge clipping alike, without touching the well-conditioned velocity computation. | `m22Holdout` generation 3 (20/20 adversarial at zero, including a ramp compounding just under the bound and a single-frame doubling); 84 consumed holdout cases now permanent regressions in `m22CvEval` |
| F3 | **The constant freeze did not cover code.** A change to a layer's normalisation order changes what the engine counts without moving any threshold, so the freeze reported itself intact while the holdout result beneath it went stale. Discovered because F2's first fix was exactly such a change. | `m22/freeze.mjs` | A ninth frozen group hashing the comment-stripped source of `detect/track/protocol/quality`. Comment-stripped rather than raw, because a freeze that cries wolf on reworded comments is one people learn to re-record without reading. | The guard fired during this work: `m22Holdout` refused to run and named `source` as the drifted group |
...
| # | Requirement | Implementation | Test | Status | Limitation |
|---|---|---|---|---|---|
| K1 | Complete session reset between attempts | `resetRun()`, a single-entry contract over `MUTABLE_RUN_KEYS` | `m22Robustness` — RESET compared field-by-field against FRESH | **done** | Adding a mutable field without adding it to the contract fails the suite, by design |
| K2 | Tracker reuse cannot leak state | `ObservationSessions({ pool })` exists to exercise the hazard rather than assume it away | `m22Robustness` | **done** | — |
| K3 | Concurrent-session isolation | Keyed by session id **only**; all layers pure, no module-level state | `m22Robustness`; `m22Perf` 1/2/5/10 interleaved | **done** | Per-session cost within ±10% of running one alone |
| K4 | Cancelled / expired / disconnected cleanup | One runtime object per session; idempotent `dispose()`; no result minted on an abandoned attempt | `m22Robustness`; `m22Perf` soak with a quarter cancelled | **done** | — |
| K5 | Dropped-frame contact boundaries | Per-triple `gapBeforeMs`/`gapAfterMs`; occlusion broken past `maxBallGap*` | `m22Robustness` | **done** | The engine does not extrapolate through lost contact frames |
| K6 | No double-count across a frame gap | Refractory in **milliseconds**, so it means the same thing across a gap | `m22Robustness` | **done** | — |
| K7 | Refractory boundary tests | 1 ms granularity either side of the declared interval | `m22Robustness` | **done** | An earlier fixture stepped 20 ms, so contacts scheduled at 179/181 ms never landed on a sample and a test "passed" with no second impulse at all — fixed as a fixture error |
| K8 | Holdout methodology | `m22/freeze.mjs` + `m22/holdout.mjs` + `scripts/m22Holdout.mjs`; generations, disjoint seeds, unseen parameter values, run once, reported separately | `m22Holdout` | **done** | Held out from *development*, not from reality |
| K9 | Named regressions | 48 named checks | `m22Robustness` | **done** | — |
| K10 | Perf, memory and soak | Per-layer timings by size and cadence, concurrency, event-loop lag vs baseline, 500-session soak, bounds | `m22Perf` | **done** | Wall-clock timings are recorded, never asserted — this is not the production machine |
| K11 | Real-world validation plan | `M22_REAL_WORLD_VALIDATION_PLAN.md` | — (it is a plan for work not done) | **done** | The work it describes has not started, and this is what holds the gate closed |
| K12 | Evaluation report | `M22_CV_EVALUATION.md` | doc↔artefact cross-check | **done** | — |
